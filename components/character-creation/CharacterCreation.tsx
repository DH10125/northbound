...
import { useId, useReducer, useRef, useEffect, useCallback, useState } from "react";
...
function draftReducer(state: DraftState, action: DraftAction): DraftState {
  switch (action.type) {
    ...
    case "SET_PRONOUNS": return { ...state, pronouns: action.value };
    case "SET_CUSTOM_PRONOUNS": return { ...state, customPronouns: action.value };
    case "SET_AGE_RANGE": return { ...state, ageRange: action.value };
    case "SET_PORTRAIT_INDEX": return { ...state, portraitIndex: action.value };
    case "SET_OCCUPATION_ID": return { ...state, occupationId: action.value };
    case "SET_MOTIVATION": return { ...state, motivation: action.value };
    case "SET_WEAKNESS": return { ...state, weakness: action.value };
    case "SET_DIFFICULTY": return { ...state, difficulty: action.value };
    case "NEXT": {
      const idx = STEPS.indexOf(state.step);
      return idx < STEPS.length - 1 ? { ...state, step: STEPS[idx + 1]! } : state;
    }
    case "BACK": {
      const idx = STEPS.indexOf(state.step);
...
function validateIdentity(draft: DraftState): string | null {
  ...
  if (draft.name.trim().length > 64) return "Name must be 64 characters or fewer.";
...
function RadioOption({ name, value, checked, onChange, label, description, badge }: RadioOptionProps) {
  return (
    <label
      className={[
        ...
        <span className="flex items-center gap-2 flex-wrap">
          ...
          {badge && (
            <span
              ...
              style={{ background: "var(--color-ember-700)", color: "var(--color-ember-200)" }}
            >
              {badge}
            </span>
          )}
...
const AGE_RANGE_OPTIONS: { value: CharacterDraft["ageRange"]; label: string }[] = [
...
function IdentityStep({ draft, dispatch, headingId, error }: IdentityStepProps) {
  const nameId = useId();
  ...
  return (
    <section aria-labelledby={headingId}>
      <div className="space-y-6">
        {/* Name */}
        <div>
          <label htmlFor={nameId} className="block text-sm font-semibold mb-1" style={{ color: "var(--text-secondary)" }}>
            Name
          </label>
          <input
            id={nameId}
            ...
            className={[
              ...
              error && draft.name.trim() === "" ? "border-[var(--interactive-danger)]" : "border-[var(--surface-border)]",
            ].join(" ")}
            style={{ color: "var(--text-primary)" }}
            aria-required="true"
            aria-describedby={error && draft.name.trim() === "" ? `${nameId}-error` : undefined}
          />
          <div className="flex justify-between mt-1">
            {error && draft.name.trim() === "" ? (
              <span id={`${nameId}-error`} role="alert" className="text-xs" style={{ color: "var(--interactive-danger)" }}>
                {error}
              </span>
            ) : <span />}
        ...
        <Fieldset legend="Pronouns">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {PRONOUN_OPTIONS.map((opt) => (
              <RadioOption
                ...
                onChange={() => dispatch({ type: "SET_PRONOUNS", value: opt.value })}
                label={opt.label}
              />
            ))}
          </div>
          {draft.pronouns === "custom" && (
            <div className="mt-3">
              <label htmlFor={customPronounsId} className="block text-xs font-medium mb-1" style={{ color: "var(--text-tertiary)" }}>
              ...
              <input
                ...
                onChange={(e) => dispatch({ type: "SET_CUSTOM_PRONOUNS", value: e.target.value })}
                maxLength={32}
                placeholder="xe/xem"
                className={[
                  ...
                  "bg-[var(--surface-overlay)] border",
                  "focus:outline-[length:var(--focus-ring-width)] focus:outline-[var(--focus-ring-color)] focus:outline-offset-[var(--focus-ring-offset)]",
                  error && draft.pronouns === "custom" && !draft.customPronouns.trim()
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {AGE_RANGE_OPTIONS.map((opt) => (
              <RadioOption
                ...
                onChange={() => dispatch({ type: "SET_AGE_RANGE", value: opt.value })}
                label={opt.label}
              />
            ))}
          </div>
        ...
        <Fieldset legend="Silhouette / portrait">
          ...
          >
            <span id={portraitGroupId} className="sr-only">Choose a portrait silhouette (index 0–7)</span>
            {Array.from({ length: PORTRAIT_COUNT }, (_, i) => (
              <label key={i} className="cursor-pointer">
                <input
                  ...
                  onChange={() => dispatch({ type: "SET_PORTRAIT_INDEX", value: i })}
                  className="sr-only"
                />
                <span
...
function OccupationCard({ occupation, selected, onSelect }: OccupationCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Attribute bars */}
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 mb-3" aria-label="Derived attributes">
        {attrEntries.map(([key, val]) => (
          <div key={key} className="flex items-center gap-2">
            <span className="text-xs w-16 capitalize flex-shrink-0" style={{ color: "var(--text-tertiary)" }}>
            ...
            <span className="text-xs w-4 text-right" style={{ color: "var(--text-tertiary)" }} aria-label={`${key} ${val} out of 10`}>
              {val}
            </span>
          </div>
        ))}
      ...
      <details className="text-xs" open={selected}>
        ...
        <div className="mt-2 space-y-2">
          <div>
            <span className="font-semibold" style={{ color: "var(--color-safe-400)" }}>Strengths</span>
            <ul className="mt-1 space-y-0.5 list-disc list-inside" style={{ color: "var(--text-tertiary)" }}>
          ...
          <div>
            <span className="font-semibold" style={{ color: "var(--color-ember-400)" }}>Tradeoffs</span>
            <ul className="mt-1 space-y-0.5 list-disc list-inside" style={{ color: "var(--text-tertiary)" }}>
...
function OccupationStep({ draft, dispatch, headingId, error }: OccupationStepProps) {
  return (
    <section aria-labelledby={headingId}>
      {error && (
        <p role="alert" className="mb-4 text-sm px-3 py-2 rounded-[var(--radius-md)]"
          style={{ background: "var(--surface-overlay)", color: "var(--interactive-danger)", border: "1px solid var(--interactive-danger)" }}>
          {error}
        </p>
      )}
      <div role="radiogroup" aria-label="Choose an occupation" className="space-y-3">
        {OCCUPATIONS.map((occ) => (
          <OccupationCard
            key={occ.id}
            occupation={occ}
            selected={draft.occupationId === occ.id}
            onSelect={() => dispatch({ type: "SET_OCCUPATION_ID", value: occ.id })}
          />
        ))}
      </div>
    </section>
...
const DIFFICULTY_OPTIONS: { value: CharacterDraft["difficulty"]; label: string; description: string }[] = [
  { value: "story", label: "Guided", description: "More information, slower resource drain, higher margins. Recommended for first runs." },
  { value: "normal", label: "Standard", description: "Intended experience: challenging but achievable with careful choices." },
  { value: "hard", label: "Unforgiving", description: "Less information, faster pressure, thinner margins. For experienced players." },
];

function MotivationStep({ draft, dispatch, headingId, error }: MotivationStepProps) {
    ...
    <section aria-labelledby={headingId}>
      <div className="space-y-6">
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          {characterRef} ({pronounLabel}) has just survived the first hours of the crisis in Pensacola.
          What drives {pronounLabel.split("/")[0]} toward Butternut — and what holds {pronounLabel.split("/")[0]} back?
        </p>

        {/* Motivation */}
        <div>
          <label htmlFor={motivationId} className="block text-sm font-semibold mb-1" style={{ color: "var(--text-secondary)" }}>
            Motivation
          </label>
          <p className="text-xs mb-2" style={{ color: "var(--text-tertiary)" }}>
            Why does {characterRef} have to reach the farm? What would be lost if {pronounLabel.split("/")[0]} failed?
          </p>
          <textarea
            id={motivationId}
            value={draft.motivation}
            onChange={(e) => dispatch({ type: "SET_MOTIVATION", value: e.target.value })}
            maxLength={256}
            rows={3}
            ...
            className={[
              ...
              "bg-[var(--surface-overlay)] border",
              "focus:outline-[length:var(--focus-ring-width)] focus:outline-[var(--focus-ring-color)] focus:outline-offset-[var(--focus-ring-offset)]",
              error && !draft.motivation.trim() ? "border-[var(--interactive-danger)]" : "border-[var(--surface-border)]",
            ].join(" ")}
            style={{ color: "var(--text-primary)" }}
            aria-required="true"
          />
          <div className="flex justify-end mt-1">
            <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>{draft.motivation.length}/256</span>
          </div>
        </div>

        {/* Weakness */}
        <div>
          <label htmlFor={weaknessId} className="block text-sm font-semibold mb-1" style={{ color: "var(--text-secondary)" }}>
            Weakness
          </label>
          <p className="text-xs mb-2" style={{ color: "var(--text-tertiary)" }}>
            What does {characterRef} struggle with — not as a stat, but as a person?
          </p>
          <textarea
            id={weaknessId}
            value={draft.weakness}
            onChange={(e) => dispatch({ type: "SET_WEAKNESS", value: e.target.value })}
            maxLength={256}
            rows={3}
            ...
            className={[
              ...
              "bg-[var(--surface-overlay)] border",
              "focus:outline-[length:var(--focus-ring-width)] focus:outline-[var(--focus-ring-color)] focus:outline-offset-[var(--focus-ring-offset)]",
              error && !draft.weakness.trim() ? "border-[var(--interactive-danger)]" : "border-[var(--surface-border)]",
            ].join(" ")}
            style={{ color: "var(--text-primary)" }}
            aria-required="true"
          />
          <div className="flex justify-end mt-1">
            <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>{draft.weakness.length}/256</span>
          </div>
        </div>
          ...
          <div className="space-y-2">
            {DIFFICULTY_OPTIONS.map((opt) => (
              <RadioOption
                ...
                onChange={() => dispatch({ type: "SET_DIFFICULTY", value: opt.value })}
                label={opt.label}
...
function ReviewRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-3 text-sm">
      <dt className="flex-shrink-0 w-32 font-medium" style={{ color: "var(--text-tertiary)" }}>{label}</dt>
...
function ReviewStep({ draft, headingId }: ReviewStepProps) {
  ...
  return (
    <section aria-labelledby={headingId}>
      <dl className="space-y-3 p-4 rounded-[var(--radius-lg)]"
        style={{ background: "var(--surface-raised)", border: "1px solid var(--surface-border-subtle)" }}>
        <ReviewRow label="Name" value={draft.name.trim() || "(none)"} />
        <ReviewRow label="Pronouns" value={pronounDisplay} />
        <ReviewRow label="Age range" value={draft.ageRange.replace(/-/g, " ")} />
        <ReviewRow label="Portrait" value={`Silhouette ${draft.portraitIndex + 1}`} />
        <ReviewRow label="Occupation" value={occupation?.name ?? "(none)"} />
        {occupation?.isChallenge && (
          <div className="text-xs px-3 py-2 rounded-[var(--radius-md)]"
            style={{ background: "var(--surface-overlay)", color: "var(--color-ember-300)" }}>
            Tourist is a deliberate challenge mode — attributes are weaker but resolve and social start higher.
            The journey is harder, the margins thinner, and completion is genuinely earned.
          </div>
        )}
        <ReviewRow label="Difficulty" value={
          ({ story: "Guided", normal: "Standard", hard: "Unforgiving" } as Record<string, string>)[draft.difficulty] ?? draft.difficulty
        } />
        <div className="pt-2 border-t" style={{ borderColor: "var(--surface-border-subtle)" }}>
          <dt className="text-sm font-medium mb-1" style={{ color: "var(--text-tertiary)" }}>Motivation</dt>
          <dd className="text-sm" style={{ color: "var(--text-primary)" }}>{draft.motivation.trim() || "(none)"}</dd>
        </div>
        <div>
          <dt className="text-sm font-medium mb-1" style={{ color: "var(--text-tertiary)" }}>Weakness</dt>
          <dd className="text-sm" style={{ color: "var(--text-primary)" }}>{draft.weakness.trim() || "(none)"}</dd>
        </div>
      </dl>
    </section>
  );
...
export function CharacterCreation({ onComplete }: CharacterCreationProps) {
  ...
  const stepError: string | null =
    state.step === "identity" ? null : // lazy validation on submit
    state.step === "occupation" ? null :
    state.step === "motivation" ? null :
    null;
  ...
  const handleBegin = useCallback(() => {
    ...
    const draft: CharacterDraft = {
      ...
      ...(state.pronouns === "custom" ? { customPronouns: state.customPronouns.trim() } : {}),
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
        ...
        <ol className="flex items-center gap-2" role="list">
          {STEPS.map((step, i) => (
            <li key={step} className="flex items-center gap-2">
              {i > 0 && (
                <span aria-hidden="true" className="w-6 h-px flex-shrink-0" style={{ background: "var(--surface-border)" }} />
              )}
              <span
                className={[
      ...
      {/* Step content */}
      {state.step === "identity" && (
        <IdentityStep draft={state} dispatch={dispatch} headingId={headingId} error={currentError} />
      )}
      {state.step === "occupation" && (
        <OccupationStep draft={state} dispatch={dispatch} headingId={headingId} error={currentError} />
      )}
      {state.step === "motivation" && (
        <MotivationStep draft={state} dispatch={dispatch} headingId={headingId} error={currentError} />
      )}
      {state.step === "review" && (
